#import "UIBezierPath+MXBZLength.h"

typedef struct MXBZBezierSubpath {
    CGPoint startPoint;
...
@implementation UIBezierPath (MXBZLength)
...
- (void)extractSubpaths:(MXBZBezierSubpath*)subpathArray
{
...
- (CGPoint)pointAtPercent:(CGFloat)t ofSubpath:(MXBZBezierSubpath)subpath {
...
- (CGFloat)mxbz_length {
	...
	MXBZBezierSubpath subpaths[subpathCount];
...
- (CGPoint)mxbz_pointAtPercentOfLength:(CGFloat)percent {
    ...
    MXBZBezierSubpath subpathContainingPoint;
	for (NSUInteger i = 0; i < subpathCount; i++) {

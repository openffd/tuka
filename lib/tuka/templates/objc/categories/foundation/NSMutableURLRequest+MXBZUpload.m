#import "NSMutableURLRequest+MXBZUpload.h"

@implementation NSMutableURLRequest (MXBZUpload)

+ (instancetype)mxbz_requestWithURL:(NSURL *)URL fileURL:(NSURL *)fileURL name:(NSString *)name {
    return [self mxbz_requestWithURL:URL fileURLs:@[fileURL] name:name];
}

+ (instancetype)mxbz_requestWithURL:(NSURL *)URL fileURL:(NSURL *)fileURL fileName:(NSString *)fileName name:(NSString *)name {
    return [self mxbz_requestWithURL:URL fileURLs:@[fileURL] fileNames:@[fileName] name:name];
}
...
+ (instancetype)mxbz_requestWithURL:(NSURL *)URL fileURLs:(NSArray *)fileURLs name:(NSString *)name {
    ...
    return [self mxbz_requestWithURL:URL fileURLs:fileURLs fileNames:fileNames name:name];
}

#import "UIButton+MXBZMiddleAligning.h"
...
- (UIImage *)mxbz_MiddleAlignedButtonImageScaleToSize:(CGSize)size
{
...
@implementation UIButton (MXBZMiddleAligning)

- (void)mxbz_middleAlignButtonWithSpacing:(CGFloat)spacing
{
    ...
    if (imageSize.width > ceilf(maxImageWidth)) {
        ...
        newImage = [self.imageView.image mxbz_MiddleAlignedButtonImageScaleToSize:CGSizeMake(maxImageWidth, imageSize.height * ratio)];
        imageSize = newImage.size;
    }
    if (imageSize.height > ceilf(maxImageHeight)) {
        CGFloat ratio = maxImageHeight / imageSize.height;
        newImage = [self.imageView.image mxbz_MiddleAlignedButtonImageScaleToSize:CGSizeMake(imageSize.width * ratio, maxImageHeight)];
        imageSize = newImage.size;
    }
    if (newImage) {
